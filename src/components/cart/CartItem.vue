<template>
  ...
  >
    ...
    <div class="flex flex-col justify-center">
      <h3 class="font-semibold text-base md:text-lg text-black">{{ item.name }}</h3>
      <p v-if="item.color || item.size" class="text-sm text-black/60">
    ...
    <div class="flex items-center justify-center gap-2">
      <button
        ...
        class="w-8 h-8 flex items-center justify-center border border-gray-300 rounded-full hover:bg-gray-100 transition-colors"
        aria-label="Уменьшить количество"
      >
        -
      </button>
      <span class="text-base font-medium w-8 text-center">{{ item.quantity }}</span>
      <button
